  ...
  if (collections.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 mb-6">Be the first to create a collection on the platform!</p>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {collections.map((collection) => (
        <Link href={`/collections/${collection.collection_id}`} key={collection.collection_id}>
          <CardContainer key={collection.collection_id} className="hover:shadow-lg transition-shadow bg-card text-card-foreground border rounded-xl">
            <CardBody className="relative group/card border-black/[0.1] w-auto h-auto rounded-xl p-2 border">
              <CardItem translateZ="100" className="w-full mt-4">
                {collection.image && !imageErrors[collection.collection_id] ? (
                  <Image
                    src={getImageUrl(collection.image, collection.collection_id)}
                    alt={collection.name}
                    width={100}
                    height={100}
              ...
              <div className="mt-4">
                ...
                <CardItem className="flex items-center justify-between gap-1.5 text-sm mt-2">
                  ...
                  <p className="font-mono text-xs truncate" title={collection.creator}>
