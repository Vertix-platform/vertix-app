...
const CreateCollectionPage = () => {
  ...
  const { uploadFile, isUploading: isIPFSUploading, clearError: clearIPFSError } = useIPFS()
  const {
    createCollection,
  ...
  const handleInputChange = (field: keyof CreateCollectionFormData, value: string | File | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleInputChange("max_supply", e.target.value)}
                  disabled={isProcessing}
                  min="1"
                  max="1000"
                />
                ...
                <div className="space-y-3">
                  ...
                  {createError && (
                    <div className="p-4 bg-red-50 border border-red-200 rounded-lg dark:bg-red-950 dark:border-red-800">
                      ...
                      <p className="text-sm text-red-700 dark:text-red-300 mt-1">{createError}</p>
                    </div>
                  )}
              ...
              <Button
                ...
                disabled={!canCreate || !formData.image || !formData.name.trim() || !formData.symbol.trim() || isProcessing || isIPFSUploading}
                className="w-full"
                size="lg"
              >
                {isProcessing || isIPFSUploading ? (
                  <>
                    ...
                    {isIPFSUploading ? 'Uploading to IPFS...' : 'Creating Collection...'}
                  </>
                ) : (
                  <>
