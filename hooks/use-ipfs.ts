...
export const useIPFS = (): UseIPFSReturn => {
  ...
  const uploadNFT = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to upload NFT to IPFS';
  ...
  const uploadFile = useCallback(
    async (file: File, config?: IPFSConfig): Promise<string | null> => {
      setIsUploading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to upload file to IPFS';
        setError(errorMessage);
        return null;
      } finally {
        setIsUploading(false);
