...
export function useCreateCollection() {
  ...
  const createCollection = useCallback(
    async (params: CreateCollectionParams): Promise<{ success: boolean; collectionId?: bigint; transactionHash?: Hash }> => {
      try {
        // Validation
        ...
        if (error instanceof Error) {
          ...
          } else if (error.message.includes('VertixNFT__ExceedsMaxCollectionSize')) {
  ...
  return {
    ...
    canCreate: isConnected && isSupportedChain(chainId) && !state.isLoading && !state.isPending,
