...
export function useCreatorNFTs() {
  ...
  const { data: allCollections, isLoading: isLoadingCollections } = useReadContract({
    address: contractAddress,
    abi: VERTIX_NFT_ABI,
    functionName: 'collections',
    args: [0n], // This would need to be implemented differently to get all collections
    query: {
  ...
  const fetchUserNFTs = useCallback(async (walletAddress: string) => {
    if (!isConnected || !isSupportedChain(chainId) || !contractAddress) {
      setError('Please connect your wallet and switch to a supported network')
      return
    }
  ...
  const fetchUserCollections = useCallback(async (walletAddress: string) => {
    if (!isConnected || !isSupportedChain(chainId) || !contractAddress) {
      setError('Please connect your wallet and switch to a supported network')
      return
    }
  ...
  const fetchCreatorStats = useCallback(async (walletAddress: string) => {
    if (!isConnected || !isSupportedChain(chainId) || !contractAddress) {
      setError('Please connect your wallet and switch to a supported network')
      return
    }
