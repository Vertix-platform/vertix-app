...
export const useAuth = () => {
  const {
    login, logout, setPrivyUser, setLoading, setError, clearError,
    user, privyUser, isAuthenticated, isLoading, error, checkAuth, refreshAccessToken,
  } = useAuthStore();
  ...
  const connectWallet = useCallback(async (walletAddress: string) => {
    try {
      ...
      const nonceResponse = await apiClient.getNonce({ wallet_address: walletAddress });
  ...
  const loginWithEmail = useCallback(async (data: LoginRequest) => {
    try {
      setLoading(true);
      clearError();
    ...
    try {
      ...
      if (!authUrlResponse.success || !authUrlResponse.data) {
        if (authUrlResponse.error?.includes('GOOGLE_CLIENT_ID must be set')) {
          throw new Error('Google OAuth is not configured. Please set up Google OAuth credentials in the backend.');
        }
  ...
  const handleGoogleCallback = useCallback(async (code: string, state: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Google OAuth callback failed';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
